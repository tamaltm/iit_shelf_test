...
class _DirectorLibraryPageState extends State<DirectorLibraryPage> {
  ...
  List<Map<String, dynamic>> _buildFromResources() {
    ...
    final start = 2; // different rotation for director page
    for (var i = 0; i < bookResources.length; i++) {
      ...
      list.add({
        ...
        'title': res['title']!,
        'author': res['author']!,
        'status': i == 0 ? 'Expected: 9/11/2025' : 'Available',
        'statusColor': i == 0 ? Colors.orange : Colors.green,
        'action': i == 0 ? 'Reserve' : 'Borrow',
      });
    }
    // append demo unavailable book
    list.add({
